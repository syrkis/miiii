...
def apply(ds: Dataset, key: Array, params: Params, x) -> Tuple[Array, Array]:
    ...
    logits = jnp.dot(x[-1], params.unbeds)
    return logits, z
...
# Init
def init_embed_fn(rng: Array, cfg) -> Embedding:
...
def init_ffwd_fn(rng: Array, cfg) -> Feedforward:
...
def init_fn(rng: Array, cfg, ds: Dataset) -> Params:
    ...
    unbeds = initializer(keys[1], (*ds.y.shape[1:], cfg.latent_dim, *(cfg.p,)))
