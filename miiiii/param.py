...
def init_head_fn(rng, cfg):
    ...
    return mi.types.Head(query=query, key=key, value=value, proj=proj)
...
def init_ffwd_fn(rng, cfg):
    ...
    return mi.types.FeedForward(w1=w1, b1=b1, w2=w2, b2=b2)


def init_block_fn(rng, cfg):
    rng, key1, key2 = random.split(rng, 3)
    params = mi.types.Block(head=init_head_fn(key1, cfg), ffwd=init_ffwd_fn(key2, cfg))
    return params
