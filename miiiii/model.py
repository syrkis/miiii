...
def attn_fn(p: mi.kinds.Attention, x: Array):
    ...
    weivp = weiv @ p.p
    return weivp, mi.kinds.Activation(q=q, k=k, v=v, qk=qk, wei=wei, weiv=weiv)


def ffwd_fn(p: mi.kinds.Feedforward, x: Array) -> Tuple[Array, Array]:
...
def init_ffwd_fn(rng: Array, cfg: mi.kinds.Conf) -> mi.kinds.Feedforward:
    ...
    return mi.kinds.Feedforward(w1=w1, w2=w2)  # , b1=b1, w2=w2, b2=b2)
...
def init_unbeds(rng: Array, cfg: mi.kinds.Conf) -> Array:
    keys = random.split(rng, 2)
...
def init_fn(rng: Array, cfg: mi.kinds.Conf):  # x: Array, y: Array) -> mi.kinds.Params:
    ...
    return mi.kinds.Params(embeds=embeds, unbeds=unbeds, blocks=blocks)
